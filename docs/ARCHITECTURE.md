...
Update state and history are stored in SQLite. Update work runs in background threads so browser/API requests return immediately and the frontend polls job progress. Update All is sequential and stops on failure. Host-update jobs use a separate persistent lifecycle. Before the disruptive action starts, the intended current/latest versions are stored. If Dashboard restarts while that operation is running or reconnecting, startup recovery does **not** reissue the update command; it resumes only readiness/version verification. This allows Dashboard to recover safely when it is itself hosted on the machine being rebooted, while avoiding a duplicate host-update request.
