...
v0.14 prefers the current versioned JSON-RPC WebSocket API (`/api/current`) for TrueNAS telemetry and management. The older REST v2 path remains only as a read-only telemetry fallback for older installations; it is not used for TrueNAS App upgrades.
